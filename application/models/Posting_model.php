...
class Posting_model extends CI_MODEL
{
    function posting($data)
    {
        foreach ($data['jenis'] as $jenis) {
            ...
            switch ($jenis) {
                case 'piutang':
                    // $tanggal = date('Y-m-01', strtotime($data['range']['tahun'] . '-' . $data['range']['bulan']));
                    $tanggal = date('Y-m-01', strtotime($data['i_periode']));
                    $tanggal_awal = new DateTime($tanggal);
                    $date_periode = $tanggal_awal->format('Y-m-d');
                    ...
                    while ($date_periode < date('Y-m-d')) {
                        echo $date_periode . "<br>";
                        $date_periode_sebelumnya = new DateTime($date_periode);
                        $date_periode_sebelumnya = $date_periode_sebelumnya->modify('-1 month');
                        $periode_sebelumnya = $date_periode_sebelumnya->format('Ym');
                        $periode = new DateTime($date_periode);
                        $periode = $periode->format('Ym');
                        // echo $periode . " Periode <br>";
                        if ($date_periode < date('Y-m-d')) {

                            //STEP AWAL
                            // hapus data periode yang mau di ubah
                            // if ($data['range']['bulan'] - 1 == 0) {
                            //     $periode_sebelumnya = $data['range']['tahun'] - 1 . 12;
                            // } else {
                            //     $peqriode_sebelumnya = $data['range']['tahun'] . str_pad($data['range']['bulan'] - 1, 2, 0, STR_PAD_LEFT);
                            // }
                            ...
                            if ($count_periode > 0) {
                                ...
                                if ($this->db->delete('piut', array('periode' => $periode))) {
                                }
                            } else {
                            }
                            //SELESAI
                            ...
                            if ($query_t_piut) {
                                ...
                                $insert_t_piutang_minv = $this->db->query("INSERT INTO t_piut (acc, type, periode, nilai, nojnl, kode, tgl, tjt, kodesls )
                                    SELECT (CASE WHEN 0 = 0 then '114.100' end) AS acc, (CASE WHEN 0 = 0 then 'FAKTUR' end) AS type, concat(extract(YEAR FROM tf),to_char(tf,'MM')) AS periode, total as nilai , nf AS nojnl , kode, tf as tgl, tjt, slskode from minv where extract(YEAR FROM tf) = " . $tahun . " and extract(MONTH FROM tf) =  " . $bulan . "");
                                // STEP 3
                                //ambil dp di tkpiut
                                $insert_t_piutang_tkpiut = $this->db->query("INSERT INTO t_piut (acc, type, periode, nilai, nojnl, kode, tgl, tjt )
                                    SELECT (CASE WHEN 0 = 0 then '215.210' end) AS acc, (CASE WHEN 0 = 0 then 'KOREKSI' end) AS type,concat(extract(YEAR FROM tgl),to_char(tgl,'MM')) AS periode, qty AS nilai, nojnl , kode, tgl, tgl as tjt FROM tkpiut WHERE  extract(YEAR FROM tgl) = " . $tahun . " and extract(MONTH FROM tgl) =  " . $bulan . "");
                                // STEP 4
                                // ambil returan di mrj
                                $insert_t_piutang_mrj = $this->db->query("INSERT INTO t_piut (acc, type,periode, nilai, nojnl, kode, tgl, tjt)
                                    SELECT (CASE WHEN 0 = 0 then '114.100' end) AS acc, (CASE WHEN 0 = 0 then 'RETUR' end) AS type, concat(extract(YEAR FROM tf),to_char(tf,'MM')) AS periode, total AS nilai, nf AS nojnl, kode, tf as tgl , tjt from mrj where extract(YEAR FROM tf) = " . $tahun . " and extract(MONTH FROM tf) =  " . $bulan . "");
                                // STEP 5
                                ...
                                $insert_t_dpiutang_dpiutang = $this->db->query("INSERT INTO t_dpiutang (refno, bayar)
                                    SELECT refno, sum(bayar) AS bayar FROM dpiutang WHERE extract(YEAR FROM tf) = " . $tahun . " AND extract(MONTH FROM tf) =  " . $bulan . " GROUP BY refno ");
                            }
                        ...
                        $tanggal_cek = new DateTime($date_periode);
                        $tanggal_cek = $tanggal_cek->modify('+1 month');
                        $date_periode = $tanggal_cek->format('Y-m-d');
                        $drop_table = $this->db->query("DROP TABLE IF EXISTS t_piut");
                        $drop_table = $this->db->query("DROP TABLE IF EXISTS t_dpiutang");
                        // echo $periode . "<br>";
